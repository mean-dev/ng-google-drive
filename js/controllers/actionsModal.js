    ...
    function($scope, $filter, $rootScope, gdisk, $mdDialog, $mdToast, Drive, $sce) {
        ...
        $scope.openwith = {
            'open' : function($event,file){
                ...
                var fileExt = fileNameArr.length > 1 ? fileNameArr[fileNameArr.length-1] : ''; // parsed file extension


                angular.forEach($rootScope.apps, function(value,key){

                    // search in primary mime types
                    var i = value['primaryMimeTypes'].indexOf($rootScope.selected['mimetype']);
                    if(i>-1){
                        value.openUrlTemplate = value.openUrlTemplate.replace("\{exportIds\}",'');
                    ...
                    // search in primary file extensions
                    if(value['primaryFileExtensions']){
                        var y = value['primaryFileExtensions'].indexOf(fileExt);
                        if(y>-1){
                            value.openUrlTemplate = value.openUrlTemplate.replace("\{exportIds\}",'');
                            value.openUrlTemplate = value.openUrlTemplate.replace("\{ids\}", file.id);
                            if($rootScope.selected['apps'].indexOf(value) == -1) $rootScope.selected['apps'].push(value);
                    ...
                    // search in secondary file extensions
                    if(value['secondaryFileExtensions']){
                        var z = value['secondaryFileExtensions'].indexOf(fileExt);
                        if(z>-1){
                            value.openUrlTemplate = value.openUrlTemplate.replace("\{exportIds\}",'');
                            value.openUrlTemplate = value.openUrlTemplate.replace("\{ids\}", file.id);
                            if($rootScope.selected['apps'].indexOf(value) == -1) $rootScope.selected['apps'].push(value);
